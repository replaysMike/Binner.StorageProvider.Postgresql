...
OFFSET {offsetRecords} ROWS FETCH NEXT {request.Results} ROWS ONLY;";
        ...
        public async Task<Part> AddPartAsync(Part part, IUserContext? userContext)
        {
            ...
            var query =
$@"INSERT INTO dbo.""Parts"" (""Quantity"", ""LowStockThreshold"", ""PartNumber"", ""PackageType"", ""MountingTypeId"", ""DigiKeyPartNumber"", ""MouserPartNumber"", ""Description"", ""PartTypeId"", ""ProjectId"", ""Keywords"", ""DatasheetUrl"", ""Location"", ""BinNumber"", ""BinNumber2"", ""UserId"", ""Cost"", ""Manufacturer"", ""ManufacturerPartNumber"", ""LowestCostSupplier"", ""LowestCostSupplierUrl"", ""ProductUrl"", ""ImageUrl"", ""DateCreatedUtc"", ""ArrowPartNumber"", ""Currency"") 
...
";
        ...
        public async Task<Pcb> AddPcbAsync(Pcb pcb, IUserContext? userContext)
        {
            ...
            var query =
                $@"INSERT INTO dbo.""Pcbs"" (""Name"", ""Description"", ""SerialNumberFormat"", ""LastSerialNumber"", ""UserId"", ""DateCreatedUtc"", ""DateModifiedUtc"", ""Quantity"", ""Cost"") 
...
";
        ...
        public async Task<Pcb> UpdatePcbAsync(Pcb pcb, IUserContext? userContext)
        {
            ...
            if (result.Any())
            {
                query = $@"UPDATE dbo.""Pcbs"" SET ""Name"" = @Name, ""Description"" = @Description, ""SerialNumberFormat"" = @SerialNumberFormat, ""LastSerialNumber"" = @LastSerialNumber, ""DateModifiedUtc"" = @DateModifiedUtc, ""Quantity"" = @Quantity, ""Cost"" = @Cost WHERE ""PcbId"" = @PcbId AND (@UserId::integer IS NULL OR ""UserId"" = @UserId);";
...
OFFSET {offsetRecords} ROWS FETCH NEXT {request.Results} ROWS ONLY;";
        ...
        public async Task<ProjectPartAssignment> AddProjectPartAssignmentAsync(ProjectPartAssignment assignment, IUserContext? userContext)
        {
            assignment.UserId = userContext?.UserId;
            var query =
                $@"INSERT INTO dbo.""ProjectPartAssignments"" (""ProjectId"", ""PartId"", ""PcbId"", ""PartName"", ""Quantity"", ""Notes"", ""ReferenceId"", ""UserId"", ""DateCreatedUtc"", ""DateModifiedUtc"", ""QuantityAvailable"", ""SchematicReferenceId"", ""CustomDescription"", ""Cost"", ""Currency"") 
...
";
        ...
        public async Task<ProjectPartAssignment> UpdateProjectPartAssignmentAsync(ProjectPartAssignment assignment, IUserContext? userContext)
        {
            ...
            if (result.Any())
            {
                query = $@"UPDATE ""ProjectPartAssignments"" SET ""ProjectId"" = @ProjectId, ""PartId"" = @PartId, ""PcbId"" = @PcbId, ""PartName"" = @PartName, ""Quantity"" = @Quantity, ""Notes"" = @Notes, ""ReferenceId"" = @ReferenceId, ""DateModifiedUtc"" = @DateModifiedUtc, ""QuantityAvailable"" = @QuantityAvailable, ""SchematicReferenceId"" = @SchematicReferenceId, ""CustomDescription"" = @CustomDescription, ""Cost"" = @Cost, ""Currency"" = @Currency WHERE ""ProjectPartAssignmentId"" = @ProjectPartAssignmentId AND (@UserId::integer IS NULL OR ""UserId"" = @UserId);";
                await ExecuteAsync(query, assignment);
            }
            else
            {
